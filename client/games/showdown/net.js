...
export const MAX_BRAWLERS = 10, MAX_HUMANS = 8, CLASS_COUNT = 4;
...
/* Build the same roster on every machine: humans sorted by id take the first slots in their avatar's colour, CPU
   ...
   The CPU names and classes come from the round's seed; a human's class is whatever they pick once the round is on. */
export function buildRoster(session) {
  ...
  const off = Math.floor(R.rnd() * CLASS_COUNT), want = fill ? MAX_BRAWLERS : Math.max(roster.length, 2);
  for (let k = 0; roster.length < want; k++) roster.push({ i: roster.length, pid: null, name: names[k], human: false, color: CPU_SHIRTS[(k + off) % CPU_SHIRTS.length], cls: (k + off) % CLASS_COUNT });
  return roster;
}

/* ---- brawlers. Flags: 1 alive, 2 in tall grass, 4 revealed (shot or was hit a moment ago), 8 bull rush, 16 human. */
export function packBrawler(b) {
  const flags = (b.alive ? 1 : 0) | (b.inGrass ? 2 : 0) | (b.reveal > 0 ? 4 : 0) | (b.dash ? 8 : 0) | (b.human ? 16 : 0);
  return [r2(b.x), r2(b.z), r2(b.dir), r2(b.mvx), r2(b.mvz), Math.ceil(b.hp), b.maxhp | 0, b.cubes | 0, r2(b.ammo), r2(b.sup), flags, b.kills | 0, b.cls | 0, b.ack | 0];
}
export function unpackBrawler(a) {
  ...
  return { x: num(a[0]), z: num(a[1]), dir: num(a[2]), mvx: num(a[3]), mvz: num(a[4]), hp: num(a[5]), maxhp: num(a[6], 1), cubes: num(a[7]), ammo: num(a[8]), sup: num(a[9]),
    alive: !!(flags & 1), inGrass: !!(flags & 2), revealed: !!(flags & 4), dash: !!(flags & 8), human: !!(flags & 16), kills: num(a[11]), cls: clamp(num(a[12]) | 0, 0, CLASS_COUNT - 1), ack: num(a[13]) };
}
