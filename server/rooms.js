/* Rooms and lobby: a game-agnostic state machine. The server never simulates anything; the host
   ...
   Lobby protocol (client -> server): create, join, lobby, opt, start, end, leave, rematch
   Lobby protocol (server -> client): joined, lobby, start, end, left, closed, error, notice, rematch
   `error` and `notice` carry a `code` (+ `vars`), never a sentence: each client words it in its own language (app.js's strings).
   The host leaving hands the room to the next player instead of closing it; mid-round that also ends the round, because the
   host's browser was running the game. A guest may vote for a rematch while a round is on; the votes go to everyone. */
...
export const CODE_ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // no I, O, 0 or 1: they read alike
...
export function createRooms({ addrHint, log = console.log }) {
  ...
  const err = (ws, code, vars) => send(ws, { t: 'error', code, vars });
  const rematchMsg = room => ({ t: 'rematch', ids: [...room.players.values()].filter(p => p.rematch).map(p => p.id) });
  function backToLobby(room) { room.state = 'lobby'; for (const o of room.players.values()) { o.ready = false; o.rematch = false; } }
  ...
  function joinRoom(ws, room, name, avatar) {
    const p = ws.meta; p.room = room; p.name = cleanName(name); p.avatar = freeAvatar(room, cleanAvatar(avatar)); p.ready = false; p.rematch = false; p.team = balancedTeam(room);
  ...
  function leaveRoom(ws) {
    ...
    if (room.players.size === 0) { rooms.delete(room.code); log(`room ${room.code} closed`); return; }
    broadcast(room, { t: 'left', id: p.id });
    if (room.hostId === p.id) { // hand the room to whoever has been in it longest (a Map keeps join order)
      const next = room.players.values().next().value; room.hostId = next.id;
      broadcast(room, { t: 'notice', code: 'newHost', vars: { name: next.name } });
      if (room.state === 'playing') { backToLobby(room); broadcast(room, { t: 'end' }); }
      log(`room ${room.code}: host left, ${next.name} now hosts`);
    } else if (room.state === 'playing') broadcast(room, rematchMsg(room));
    broadcast(room, lobbyMsg(room));
  }
  ...
  function onMessage(ws, msg) {
    ...
    switch (msg.t) {
      case 'create': {
        const game = gameById(msg.game); if (!game) { err(ws, 'unknownGame'); break; }
        const want = String(msg.room || '').trim().toUpperCase(); // the host may pick the code; empty means a random one
        if (want && !CODE_RE.test(want)) { err(ws, 'badCode'); break; }
        if (rooms.has(want)) { err(ws, 'roomExists', { code: want }); break; }
      ...
      case 'join': {
        ...
        if (!r) { err(ws, 'noRoom', { code: code || '?' }); break; }
        if (r.state !== 'lobby') { err(ws, 'running'); break; }
        if (r.players.size >= r.game.maxPlayers) { err(ws, 'full'); break; }
      ...
      case 'lobby': {
        ...
        if (msg.team !== undefined && room.game.teams) { // switch sides (until READY); a side holds at most teamSize players
          ...
          if (t && t.id !== p.team && !p.ready) { if (teamCount(room, t.id) >= (room.game.teamSize || Infinity)) err(ws, 'teamFull', { team: t.id }); else p.team = t.id; }
        }
      ...
      case 'start': {
        if (!isHost) break;
        if (room.players.size < room.game.minPlayers) { err(ws, 'needPlayers', { n: room.game.minPlayers }); break; }
        room.state = 'playing'; for (const o of room.players.values()) o.rematch = false;
      ...
      case 'end': if (isHost) { backToLobby(room); broadcast(room, { t: 'end' }); broadcast(room, lobbyMsg(room)); } break;
      case 'rematch': if (room && room.state === 'playing' && !isHost) { p.rematch = msg.on !== false; broadcast(room, rematchMsg(room)); } break;
  ...
  function attach(wss) {
    wss.on('connection', ws => {
      ws.meta = { id: nextId++, ws, room: null, name: 'Player', avatar: 0, ready: false, rematch: false, team: null }; ws.isAlive = true;
